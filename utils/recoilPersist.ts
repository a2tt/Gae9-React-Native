 ...
 * https://github.com/polemius/recoil-persist/blob/master/src/index.ts
 */
...
export const recoilPersist = (
  ...
  const getState = (): any => {
    ...
    if (toParse === null || toParse === undefined) { // XXX equation fixed
      return {}
    }
    ...
    if (typeof toParse.then === 'function') {
      return toParse.then(parseState).catch(e => console.error(e))
    }
