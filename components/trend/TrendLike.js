...
const LikeItem: () => Node = ({type, expression, onPress, myExpression}) => {
  myExpression = myExpression === undefined ? 1 : myExpression; // TODO DELETE
  ...
  const cnt = expression[targetKey];
  const imageData = type === 1 ? good : bad;
  const opacity = myExpression >= 1 ? 1 : 0.2;

  // TODO onPress
  return (
    <LikeTouchableOpacity onPress={e => onPress(expression)}>
      <LikeView>
        <LikeImage
          resizeMode={'contain'}
...
export const TrendLikeContainer: () => Node = ({expression, setExpression}) => {
  return (
    <TrendLikeView>
      <LikeItem type={1} expression={expression} onPress={setExpression}/>
      <LikeItem type={-1} expression={expression} onPress={setExpression}/>
    </TrendLikeView>
  );
};
